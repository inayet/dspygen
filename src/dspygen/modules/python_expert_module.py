...
class PythonExpertModule(dspy.Module):
    ...
    def forward(self, user_story, skill_level, has_comments):
        pred = dspy.Predict(
            "user_story, skill_level, has_comments -> python_source_code"
        )
        result = pred(
            user_story=user_story, skill_level=skill_level, has_comments=has_comments
        ).python_source_code
        return result
...
def python_expert_call(user_story, skill_level, has_comments: bool):
    ...
    return python_expert.forward(
        user_story=user_story, skill_level=skill_level, has_comments=str(has_comments)
    )
