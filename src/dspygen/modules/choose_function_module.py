...
def functions_to_dict(funcs: list[Callable]) -> dict:
    return {func.__name__: function_to_dict(func) for func in funcs}
...
class ChooseFunctionModule(dspy.Module):
    ...
    def validate_output(self, chosen_function_name: str) -> bool:
        ...
        valid_choice = (
            chosen_function_name in self.functions_dict
            if self.functions_dict
            else False
        )
        dspy.Assert(
            valid_choice,
            f"Invalid function name chosen: {chosen_function_name}. "
    ...
    def forward(self, prompt, function_list: list[Callable] = None) -> Callable:
        ...
        matching_function__name__ = pred(
            prompt=prompt, function_list=str(self.functions_dict)
        ).matching_function__name__

        try:
            if self.validate_output(matching_function__name__):
                return next(
                    filter(
                        lambda f: f.__name__ == matching_function__name__,
            ...
            pred = dspy.ChainOfThought(
                "prompt, function_list, error -> matching_function__name__"
            )
            matching_function__name__ = pred(
                prompt=prompt, function_list=function_list, error=str(e)
            ).matching_function__name__
            ...
            if self.validate_output(matching_function__name__):
                ...
                raise ValueError(
                    f"Invalid function name chosen: {matching_function__name__}"
                )
